...
function renderQueue(q){$("queue").innerHTML=q.length?q.map(x=>`<tr><td>${escapeHtml(x.name)}</td><td><span class="badge ${x.status}">${x.status}</span><div>${escapeHtml(x.message||"")}</div></td><td><div class="bar"><i style="width:${x.progress||0}%"></i></div>${x.progress||0}%</td><td>${x.output?escapeHtml(x.output):"-"}</td></tr>`).join(""):`<tr><td colspan="4">Belum ada video. Pilih folder lalu klik Scan.</td></tr>`;const done=q.filter(x=>x.status==="done").length,fail=q.filter(x=>x.status==="failed").length;$("total").textContent=q.length;$("done").textContent=done;$("failed").textContent=fail;$("progress").textContent=q.length?Math.round(q.reduce((a,x)=>a+(x.progress||0),0)/q.length)+"%":"0%"}
function escapeHtml(s){return String(s).replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[m]))}
async function refresh(){const j=await api("/api/state");renderQueue(j.queue);$("current").textContent=j.state.running?`Sedang memproses: ${j.state.current||"menyiapkan…"}`:"Belum ada produksi aktif.";$("log").textContent=(j.state.log||[]).join("\n")||"AIOS siap.";$("startBtn").classList.toggle("hidden",j.state.running);$("stopBtn").classList.toggle("hidden",!j.state.running)}
...
async function check(){const j=await api("/api/check");$("healthDot").className="dot "+(j.ffmpeg?"ok":"bad");$("healthText").textContent=j.ffmpeg?"Sistem siap":"FFmpeg belum ada";$("healthSub").textContent=j.ffmpeg?`${j.node} · FFmpeg aktif`:"Jalankan install-ffmpeg.bat"}
...
async function openOutput(){if(!window.aiosDesktop)return toast("Fitur ini tersedia di versi desktop");const result=await window.aiosDesktop.openPath($("outputFolder").value);if(result)toast(result)}
