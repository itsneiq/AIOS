...
function clampCount(value, max = 5) {
...
function rotate(list, seed, offset = 0) {
  return list[(hash(seed) + offset) % list.length];
}

function pool(angle, key) {
  return ANGLES[angle]?.[key] || ANGLES.balanced[key];
}
...
function generateScript(options = {}) {
  ...
  return {
    hook: rotate(pool(angle, "hooks"), seed, 0),
    benefit: rotate(pool(angle, "benefits"), seed, 1),
    cta: rotate(pool(angle, "ctas"), seed, 2),
    angle
  };
}

function resolveScript(config = {}, item = {}, index = 0, variant = 1) {
  if (config.scriptMode !== "auto") {
    return {
      hook: String(config.hook || "").trim(),
      benefit: String(config.benefit || "").trim(),
      cta: String(config.cta || "").trim(),
      angle: "manual"
    };
  }
  return generateScript({
    angle: config.scriptAngle,
    seed: item.base || item.name || "video",
    index,
    variant
  });
}

function generateScriptSet(options = {}) {
  const count = clampCount(options.count);
  return Array.from({ length: count }, (_, index) => generateScript({
    ...options,
    index,
    variant: index + 1
  }));
