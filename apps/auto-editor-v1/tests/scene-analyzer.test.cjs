...
assert.equal(sceneDetectionFilter(0),"select='gt(scene,0.05)',showinfo");
const parsed=parseSceneLog("n:1 pts_time:1.25\nn:2 pts_time:3.5",5);
assert.deepEqual(parsed.map(x=>[x.start,x.end]),[[0,1.25],[1.25,3.5],[3.5,5]]);
assert.deepEqual(parseSceneLog("pts_time:3.5\npts_time:1.25\npts_time:1.25",5).map(x=>[x.start,x.end]),[[0,1.25],[1.25,3.5],[3.5,5]]);
assert.deepEqual(parseSceneLog("",0),[]);
...
assert.deepEqual(analyzeScenes(),{duration:0,sceneCount:0,usableCount:0,bestScene:null,scenes:[]});
const normalized=analyzeScenes({scenes:[{start:"1",end:"2.5",duration:99}]});
assert.deepEqual(normalized.scenes[0],{start:1,end:2.5,duration:1.5,index:0,type:"general",camera:"medium",quality:50,motion:50,usable:true});
