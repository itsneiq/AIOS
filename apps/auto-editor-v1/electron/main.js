...
function waitForServer(url, timeoutMs = 15000) {const started=Date.now();return new Promise((resolve,reject)=>{const probe=()=>{const req=http.get(url,res=>{res.resume();resolve()});req.on("error",()=>{if(Date.now()-started>timeoutMs)return reject(new Error("AIOS server gagal dijalankan."));setTimeout(probe,250)});req.setTimeout(1500,()=>req.destroy())};probe()})}
function stopProcessTree(child) {if(!child||child.killed)return;if(process.platform==="win32"&&child.pid)spawn("taskkill",["/pid",String(child.pid),"/t","/f"],{windowsHide:true});else child.kill("SIGTERM")}
function runtimeDir(){const dir=path.join(app.getPath("userData"),"runtime-data");fs.mkdirSync(dir,{recursive:true});return dir}
...
function startLocalServer(){const appRoot=app.isPackaged?process.resourcesPath:path.resolve(__dirname,"..");const serverPath=path.join(appRoot,"server.js"),dataDir=runtimeDir();serverProcess=spawn(process.execPath,[serverPath],{cwd:appRoot,env:{...process.env,ELECTRON_RUN_AS_NODE:"1",PORT:String(PORT),AIOS_DATA_DIR:dataDir},windowsHide:true,stdio:"pipe"});serverProcess.stdout.on("data",data=>console.log(`[AIOS] ${data}`));serverProcess.stderr.on("data",data=>console.error(`[AIOS] ${data}`))}
async function createWindow(){startLocalServer();mainWindow=new BrowserWindow({width:1440,height:920,minWidth:1100,minHeight:700,title:"AIOS Auto Editor",backgroundColor:"#0f1115",autoHideMenuBar:true,webPreferences:{preload:path.join(__dirname,"preload.js"),contextIsolation:true,nodeIntegration:false}});try{await waitForServer(`http://127.0.0.1:${PORT}`);await mainWindow.loadURL(`http://127.0.0.1:${PORT}`)}catch(error){dialog.showErrorBox("AIOS gagal dibuka",error.message);app.quit()}mainWindow.webContents.setWindowOpenHandler(({url})=>{shell.openExternal(url);return{action:"deny"}})}
